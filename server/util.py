...
def _deserialize(data, klass):
    ...
    if klass in (int,) or klass in (float, str, bool):
        ...
        elif klass.__origin__ == dict:
            return _deserialize_dict(data, klass.__args__[1])
        else:
            logger.error(
                f"Could not identify klass type. Klass.__origin__ is {klass.__origin__}",
            )
...
def deserialize_model(data, klass):
    ...
    for attr, attr_type in instance.swagger_types.items():
        ...
        if (
            data is not None
            and attr in instance.attribute_map
